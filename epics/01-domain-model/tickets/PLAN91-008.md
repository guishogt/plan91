...
- [x] HabitPractitionerId value object created with validation
...
- [x] All tests pass when run individually
- [x] Code compiles with `mvn clean compile`
- [x] No framework dependencies (pure Java)

## Technical Requirements

### Value Objects to Create

#### 1. HabitPractitionerId
**Purpose**: Unique identifier for HabitPractitioner
...
public record HabitPractitionerId(UUID value) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("✅ All HabitPractitionerId tests passed!");
    }
}
```
...
Same structure as HabitPractitionerId (UUID-based).
...
```bash
mkdir -p src/main/java/com/ctoblue/plan91/domain/habitpractitioner
...
1. Create HabitPractitionerId.java
2. Create Email.java
3. Create HabitId.java (copy pattern from HabitPractitionerId)
4. Create RoutineId.java (copy pattern from HabitPractitionerId)
5. Create HabitEntryId.java (copy pattern from HabitPractitionerId)
6. Create CategoryId.java (copy pattern from HabitPractitionerId)
...
# Test HabitPractitionerId
java -cp target/classes com.ctoblue.plan91.domain.habitpractitioner.HabitPractitionerId

# Test Email
java -cp target/classes com.ctoblue.plan91.domain.habitpractitioner.Email
...
1. **HabitPractitionerId** (`domain/habitpractitioner/HabitPractitionerId.java`)
...
2. **Email** (`domain/habitpractitioner/Email.java`)
...
$ java -ea -cp target/classes com.ctoblue.plan91.domain.habitpractitioner.HabitPractitionerId
✅ All HabitPractitionerId tests passed!

$ java -ea -cp target/classes com.ctoblue.plan91.domain.habitpractitioner.Email
...
✅ All criteria satisfied. Ready for next ticket (PLAN91-009: Create HabitPractitioner aggregate).
